...
func ActualizaConvenio(w http.ResponseWriter, r *http.Request) {
	...
	_, status, err := bbdd.ModificaConvenio(t)
	if err != nil {
		http.Error(w, "Se produjo en error en la actualización del registro en la BD:"+err.Error(), 400)
		return
	}
	if status == false {
		http.Error(w, "No se pudo actualizar el convenio en la BD:"+err.Error(), 400)
		return
	}
